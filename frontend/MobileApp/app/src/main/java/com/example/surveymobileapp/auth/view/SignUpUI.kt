...
@Composable
fun SingUpUi(paddingValue: PaddingValues, viewModel: AuthVM ){
    ...
    Column(
        ...
        val initialNameUiState = rememberTextFieldState(viewModel.nameLastString.collectAsState().value)
        val initialEmailUiState = rememberTextFieldState(viewModel.emailString.collectAsState().value)
        val initialPasswordUiState = rememberTextFieldState(viewModel.passwordString.collectAsState().value)

        Row(
            Modifier.fillMaxWidth(),
            horizontalArrangement = Arrangement.Center
        ){
            Text(fontSize = 30.sp,text="SignUp")
        }

        TextField(
            state = initialNameUiState,
            label = { Text("Name") },

        )
        TextField(
            state = initialEmailUiState,
            label = { Text("Email") },
        ...
        Button(
            onClick = {
                ...
                viewModel.updateNameLastString(initialNameUiState.text.toString())
                viewModel.updateEmailString(initialEmailUiState.text.toString())
                viewModel.updatePasswordString(initialPasswordUiState.text.toString())
                println("Evaluating input field")

                if(viewModel.checkAttributesValidity() == true){

                    val user = UserSignup(viewModel.nameLastString.value,
                        viewModel.emailString.value,
                        viewModel.passwordString.value)
                    println("Creating new user : $user")
                    CoroutineScope(Dispatchers.IO).launch{
                        val response = client.signup("http://192.168.1.37:8080/session/signup",
                           user
                        )
                    ...
                    println("SignUp failed, user must check credentials validity")
                }
            },
            modifier = Modifier.size(200.dp, 50.dp)) {
            Text(fontSize = 18.sp, text = "SignUp")
        }
        Row(
            ...
            Text("Try To LogIn")
            Icon(FontAwesomeIcons.Solid.ArrowRight,
...
fun SingUpUiPreview(){
    SingUpUi(PaddingValues(20.dp), viewModel())
}